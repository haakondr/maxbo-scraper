...
(async () => {
    ...
    const cookieButton = await findAndWaitFor(page, '[aria-label="Godkjenn alle"]')
    await cookieButton.click();

    const [availabilityButton] = await findAndWaitForXpath(page, "//button[contains(., 'butikker')]");
    await availabilityButton.click();

    await page.waitForSelector("[city=Oslo]");

    const stores = await page.$$eval("[city=Oslo]", els => els.map(el => el.innerHTML));
